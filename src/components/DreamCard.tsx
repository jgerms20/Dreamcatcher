...
export const MOOD_META: Record<number, { label: string; emoji: string; color: string }> = {
  ...
  [0]: { label: 'Neutral', emoji: '🌙', color: 'var(--color-moon-300)' },
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-xs text-ivory-300">{fmtDate(dream.dreamDate)}</p>
          <h3 className="font-display mt-0.5 truncate text-lg font-semibold text-ivory-100">
            {dream.title || 'Untitled dream'}
          </h3>
        </div>
        <div className="flex shrink-0 items-center gap-1.5 text-ivory-300">
          {dream.videoId || dream.videoUrl ? <span title="Has video"><Icon name="film" size={15} /></span> : null}
          {dream.audioId ? <span title="Has audio"><Icon name="mic" size={15} /></span> : null}
          {dream.lucid ? <span title="Lucid dream"><Icon name="eye" size={15} /></span> : null}
          {dream.recurring ? <span title="Recurring dream"><Icon name="repeat" size={15} /></span> : null}
          {mood ? <span title={mood.label}>{mood.emoji}</span> : null}
        </div>
      </div>
      <p className="font-prose mt-2 line-clamp-3 text-sm leading-relaxed text-ivory-200">{dream.transcript}</p>
